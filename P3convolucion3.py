...
saw = signal.sawtooth(2*np.pi*f*t)    # [-1,1]
saw_n = (saw+1)/2                  # [0,1]  <<— igual que en Paso 1
...
y = np.convolve(saw_n, x_exp_dec, mode='full') * dt
t_y = np.arange(t[0]+t[0], t[-1]+t[-1]+dt, dt)[:len(y)]

# --- Gráficos generales ---
plt.figure(figsize=(10,7))
plt.subplot(3,1,1); plt.plot(t, saw_n);      plt.title('x(t): cuadrada periódica (0–1)');       plt.grid(True)
